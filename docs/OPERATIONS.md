...
![ログの一覧画面。「直近◯件を新しい順に表示しています。」の下に、やりとりのカードが数件。一部に赤い「要対応」バッジ、各カードに日時・お客様の質問・自動返信・確信度（お客様の記号はぼかす）](images/IMG_6262.PNG)
...
「未撮影」の3点（No.2 / No.16 / No.19）は、本文では `📷` マーカーのまま残しています。
...
| 14  | 4.1 | ログの一覧画面（やりとりのカードが数件。1件は「要対応」バッジ付き。日時・質問・自動返信・確信度が見えるように）                | `IMG_6262.PNG`                            |
