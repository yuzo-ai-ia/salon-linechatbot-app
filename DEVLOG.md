...
1. 追加・編集フォーム（`MenuForm.tsx`・`createMenuAction`/`updateMenuAction`）。
2. 削除（`deleteMenuAction`・削除確認画面）。
3. 並び替え（`moveMenuAction`・一覧の↑↓ボタン）。
4. `/admin/faq`⇔`/admin/menus`の切り替えナビゲーション追加。
