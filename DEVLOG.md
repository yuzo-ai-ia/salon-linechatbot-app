...
## フェーズ14: README.md 差し替え（2026-09-06）

教材Step4の納品物3点のうち、まず`README.md`だけを差し替えた（`create-next-app`の
英語雛形のまま放置されていた）。セットアップ手順書・オーナー向け運用マニュアルは次回。

### やったこと

- Plan Modeで「見出し構成（誰が読むか・何を載せるか）」だけ先に確定させ、
  ユーザー確認後に本文を執筆する段取りにした。
- 想定読者は「GitHubでこのリポを見る開発者」。言語は他ドキュメントに合わせて日本語。
- 構成: 概要 / 主な機能（自動応答・要対応エスカレーション・管理画面）/ 技術構成
  （表 + Next.js 16 の注意）/ 処理の流れ（テキスト図）/ ディレクトリ構成 /
  セットアップ（前提〜起動確認の5ステップ）/ 環境変数（表・名前と役割のみ）/
  動作確認用スクリプト / デプロイ（Vercel）/ 開発ログへのリンク。
- 環境変数は`lib/env.ts`と`.env.local.example`から名前・役割・必須任意・
  `NEXT_PUBLIC`可否を表にした。**実値・キーは一切載せない**方針を明記。

### 検証

- `README.md`を`sk-` / `sb_secret` / `sb_publishable` / `eyJ` でgrep → ヒットなし。
- 記載コマンド（`npm install` / `npm run dev` / `try:answer` / `try:webhook`）が
  `package.json`の`scripts`と一致することを確認。
- 参照パス（`supabase/migrations/`の3ファイル、`proxy.ts`等）が実在することを確認。
- 疎通確認ページの表示文言（`app/page.tsx`）、`try:webhook`の`--bad-signature`
  オプションの実在も確認。

### 学び

- 納品ドキュメントも「見出しレベルだけ先に合意 → 本文」の段取りにすると、
  分量の大きい書き直しの手戻りが減る。特にREADMEは載せる/載せないの線引き
  （手順書レベルの詳細をどこまで書くか）が人によって想定が違うので、
  構成の合意を挟む価値が大きかった。

### 次回（教材Step4の残り）

1. セットアップ手順書（別オーナーが1から構築する手順。読者レベルは未確定）。
2. オーナー向け運用マニュアル（管理画面の使い方・要対応通知への対応・一斉配信の注意）。

---

## フェーズ15: セットアップ手順書（docs/SETUP.md）着手（2026-09-06）

`docs/SETUP.md` を新規作成。想定読者は「構築担当の開発者（Supabase/Vercel/LINE Consoleの基本操作は既知）」。Plan Modeで全体構成（1〜10章）を合意し、1〜5章（この手順書について／全体像／事前準備／Supabase／OpenAI）を執筆。6〜10章（LINE／ローカル確認／本番デプロイ／トラブルシューティング／付録）は次回。本番デプロイのハマり（APP_URLは再デプロイ必要、PGRST303=時計ズレ）は8・9章で反映予定。
...
## フェーズ19: オーナー向け運用マニュアル（docs/OPERATIONS.md）完成（2026-09-07）

オーナー向け運用マニュアル `docs/OPERATIONS.md` に着手。manual mode（章ごと確認）で 0章「まず読む」・1章「管理画面を開く/ログイン」・2章「FAQ管理」を執筆。用語は「自動返信」に統一（bot の仕組みは付録Bで一度だけ説明予定）、ホーム画面追加は iPhone/Android 両方記載、実画面のフラッシュ/エラー文言・文字数制限・二段階削除フローをコードと突き合わせ。📷 マーカーでスクショ差し込み位置を明示。残り = 3〜8章＋付録A/B。コミット前に `sk-`/`sb_secret`/`sb_publishable`/`eyJ`/`U<hex>` でgrep→実キー・パスワードの混入なし（URL/パスワード欄はすべて空欄プレースホルダ）。

### 追記: 3〜5章 執筆（2026-09-07）

- 全体の章立てを確定（0〜2 + 3メニュー管理 / 4ログ / 5要対応 / 6自動返信がおかしいときに直す / 7一斉配信 / 8困ったときは / 9日々の運用 + 付録A/B）。
- **3章「メニュー管理」**: `app/admin/(protected)/menus/` 一式・`MenuForm.tsx`・`menus/actions.ts` と突き合わせ。価格入力ルール（数字のみ・全角/カンマは保存時に正規化・上限 2147483647）、実エラー文言、並び替え↑↓（先頭↑・末尾↓は無効）、新規は末尾に入る、非表示（`is_active`）にすると `loadKnowledgeBase()` の `.eq("is_active", true)` で自動返信からも消える、料金改定は FAQ 回答文と2か所直す、を反映。
- **4章「ログ」**: `conversations/page.tsx`。読み取り専用・直近50件・新しい順・カテゴリ/検索/絞り込みなし。`line_user_id` は「お客様の記号（名前は無い）／同じ記号＝同じ人」、confidence は自己申告の目安（40%未満は要注意）と説明。プライバシー注意（スクショ共有しない）を明記。
- **5章「要対応」**: `lib/line/notify-owner.ts` の通知本文（【要対応】…確信度…/admin/conversations、質問は先頭200字、`APP_URL` 有無で URL/パス表記が変わる、`LINE_TEST_USER_ID` 未設定なら通知が飛ばない＝ログ画面を自分で見に行く）。返信は「LINE公式アカウント」アプリの「チャット」から手動、お客様は質問文＋時刻で照合（名前が無いため）、システム側に「対応済み」操作は無い、を反映。
- コミット前 grep（`sk-`/`sb_secret`/`sb_publishable`/`eyJ`/`U<hex>`）→ 混入なし。`git diff --stat` は `docs/OPERATIONS.md` のみ（+300行）。残り = 6〜9章＋付録A/B。

### 追記: 6〜7章 執筆（2026-09-07）

- **6章「自動返信がおかしいときに直す」**: 内容品質のチューニング章（システム障害の8章とは別）。直す場所は 2章 FAQ / 3章 メニューだけ、という一点に集約。症状べつ早見表（古い情報→回答文修正、"確認します"ばかり→FAQ未登録、組み合わせに弱い→combinationカテゴリ追加 等）。「反映は保存した瞬間から即時」「直したら自分でテスト送信」を強調。自分のLINE（=公式アカウントを友だち追加したもの）からのテスト手順。
- **7章「お知らせの一斉配信」**: `components/BroadcastForm.tsx`・`broadcasts/actions.ts`・`broadcasts/page.tsx` と突き合わせ。compose→confirm の2フェーズ、実文言（「配信する文面を入力してください。」「配信すると取り消せません。内容をよく確認してください。」「テスト送信しました。自分のLINEを確認してください。」「配信しました。」「配信に失敗しました。…」）、5000字上限、`LINE_TEST_USER_ID` 未設定時はテスト送信ボタンが出ない、赤い「配信する（友だち全員）」、配信履歴（最大50件・成功/失敗バッジ・失敗理由は英語）。冒頭に⚠️「取り消せない・相手を選べない・月の無料送信上限」を明記。
- コミット前 grep 同上→混入なし。残り = 8章（困ったときは）・9章（日々の運用）＋付録A（スクショ撮影リスト）・付録B（自動返信の仕組み／ことばの説明）。

### 追記: 8〜9章・付録A/B 執筆、マニュアル完成（2026-09-07）

- **8章「困ったときは」**: 内容の不調（6章）と切り分けて、仕組みそのものの不調（開けない・保存できない・止まった）を扱う。「まず試す3つ（更新／開き直し／通信確認）」→ 症状べつ対処表 → 構築担当者へ連絡するとき伝えること（日時・操作・画面の文言・自分で試したこと）。「自動返信が全部止まった」「ログインできない」は自分で抱えず即連絡、と明記。
- **9章「日々の運用」**: 頻度べつ早見表（毎日=要対応通知チェック／週1=ログ流し見→FAQ改善／情報変更時=該当FAQ・メニュー修正／月1=FAQ棚卸し・配信履歴確認／さわらない=パスワード・設定）。
- **付録A**: 本文の `📷` を全19点、章番号つきの撮影リストに集約。ログ撮影時は個人情報をぼかす注記。
- **付録B**: B.1「自動返信の仕組み」（FAQ＋表示中メニューを全部読む→AIが作文→載っていないことは答えない→自信がなければ要対応→数秒で返信→スタンプ/画像/空は無反応）。B.2 用語集15語（自動返信・FAQ・カテゴリ・確信度・一斉配信・テスト送信・構築担当者 等）。本文で「自動返信」に統一していた「bot」の説明はここで一度だけ回収。
- 完成時チェック: 見出し一覧を出力して 0〜9章＋付録A/B の連番・各章末「覚えておくこと」の有無・章相互参照（0章→2/5/6/7章、1.4表→2/3/4/7章）の整合を確認。grep（`sk-`/`sb_secret`/`sb_publishable`/`eyJ`/`U<hex>`）→混入なし。全888行。`git diff --name-only` は `docs/OPERATIONS.md`・`DEVLOG.md` のみ。

### 学び

- 「同じことを別の章で二度説明しない」を徹底するには、用語（今回は「自動返信」）を1章で定義し、仕組みの解説を付録に1回だけ置いて、本文各所からは章番号で参照する形にすると、読者が迷わず・書き手も更新箇所が1か所で済む。
- スマホ専用・非エンジニア向けの手順書は、UI 文言をコードから一字一句拾う（「保存に失敗しました。」等）のが効く。読者が画面で見る文字とマニュアルの文字が完全一致していると、「今どこの話か」で迷わない。
- トラブル対応章は「内容がおかしい（6章）」と「仕組みが動かない（8章）」を最初に分けると、読者が正しい章に最短でたどり着ける。混ぜると「返信が変」で全部の対処を読まされることになる。

これでフェーズ19（オーナー向け運用マニュアル）は完了。教材 Step4 の納品ドキュメント3種（SETUP / ARCHITECTURE / OPERATIONS）がすべて揃った。

### 次にやること

- 実際の管理画面スクリーンショット19点を撮影して `docs/OPERATIONS.md` の `📷` 位置に差し込む（付録A がチェックリスト）。
- 0章の連絡先・URL 欄はオーナーごとに手書きで埋める運用（テンプレートは空欄のまま）。

---

## フェーズ20: OPERATIONS.md にスクショ16枚差し込み、マニュアル完成（2026-09-07）

フェーズ19で `📷` マーカーだけ置いていた箇所に、実機（iPhone Safari／LINE アプリ）で
撮った管理画面スクリーンショットを差し込んだ。

### やったこと

- 実機スクショ 22枚を受領（`IMG_6243`〜`IMG_6271`、欠番あり）。当初プロジェクト直下の
  `images/` に置かれていたので `docs/images/` へ移動（本文が `docs/` 配下なので
  相対参照を `images/ファイル名` に統一できる）。
- 22枚を1枚ずつ目視し、付録A の撮影リスト19番と突き合わせて対応表を作成・ユーザー確認。
  - **本採用17ファイル**（No.4 と No.5 は同一画面なので `IMG_6245` を共用、
    No.5 と No.13 は補助1枚ずつ追加 = 本文の `![]()` は18行）。
  - **未撮影3点はマーカーのまま**: No.2 Android「ホーム画面に追加」／
    No.16 公式アカウントの「チャット」画面／No.19「配信しました。」表示。
  - 未使用5枚（重複・予備）は `docs/images/` に残置（消さない判断）。
- 各 `📷` ブロックの直後に `![説明](images/IMG_xxxx.PNG)` を挿入。alt テキストは
  画面の要素を日本語で具体的に（読み上げ・画像非表示時のフォールバック用）。
- 付録A の表に「画像ファイル」列を追加。未撮影3点は「未撮影（マーカーのまま）」と明記。
- パス検証: 本文の全 `images/...` 参照18行について実ファイルの存在を確認
  （`MISSING` ゼロ）。相対パスは `docs/OPERATIONS.md` 基準で `docs/images/` を正しく指す。

### 機密チェック（コミット前）

- 本文 grep（`sk-`/`sk_live`/`sb_secret_`/`sb_publishable_`/`eyJ`/`xoxb-`/`bearer`/
  40字以上の連続文字列/`password|token|secret [:=]`/電話番号/`@gmail`/`U<hex>`）
  → ヒントなし（唯一のヒットは Markdown 表の区切り線 `| --- |` の誤検出）。
- 画像22枚の目視: API キー（`sk-`/`sb_`）・パスワード・LINE channel access token /
  channel secret の写り込み **なし**。ログイン画面（`IMG_6244`）はパスワード欄が空。
- **本番URL** `salon-linechatbot-app.vercel.app` が Safari の URL バーと要対応通知の
  本文に写り込み → 既に `DEVLOG.md` に3か所コミット済み・パスワード保護付きの
  公開 Vercel URL なので新規露出なし・許容。
- **ログ画面スクショ（`IMG_6261`/`IMG_6262`）に `line_user_id` がぼかしなしで表示** →
  ユーザー本人のテスト用アカウント（`LINE_TEST_USER_ID` の値）であり実在の顧客では
  ないこと、userId 単体では何もできない（push には別途サーバー秘密の token が必要）
  こと、private リポジトリであることを確認のうえ、ユーザー判断でこのままコミット。

### 学び

- スクショの alt テキストは「画面名」だけでなく「その画面に写っている要素」
  （ボタンの文言・バッジ・見出し）まで書くと、画像が表示されない環境でも本文の
  手順と突き合わせられる。撮影リスト（付録A）の説明文がほぼそのまま流用できた。
- 画像の「置き場所」は本文ファイルからの相対パスが素直になる場所を先に決める。
  今回は `docs/OPERATIONS.md` から `images/...` で参照したいので `docs/images/` 一択。
  プロジェクト直下 `images/` のままだと `../images/...` になって移植性が下がる。
- スクショに写る「消せない情報」（URL バー・通知本文の URL・ログの userId）は、
  撮影後に気づいても加工が要る。撮る前に「テストデータで撮る」「URL バーを隠す」
  「ぼかす前提の箇所を決める」を撮影リスト側に書いておくと後工程が楽。

これで **教材 Step4 の納品ドキュメント4種（README / SETUP / ARCHITECTURE / OPERATIONS）が
すべて完成**。OPERATIONS はスクショ入りで、残タスクは未撮影3点（撮れたら差し替え）と
オーナーごとの連絡先・URL 記入のみ。

### 追記: 納品前の相互リンク整備＋公開化に伴う画像削除（2026-09-07）

- **4ドキュメントの相互リンクを整備**（README ⇔ SETUP ⇔ ARCHITECTURE ⇔ OPERATIONS）。
  - README: 概要に公開 URL、管理画面セクションにスクショ2枚（`IMG_6245`/`IMG_6255`）＋
    OPERATIONS への案内、末尾「開発ログ」→「ドキュメント」表（4ドキュメント＋DEVLOG＋CLAUDE）。
  - SETUP / ARCHITECTURE: 冒頭「関連ドキュメント」に OPERATIONS 行を追加
    （SETUP には ARCHITECTURE 行も欠けていたので併せて追加）。
  - OPERATIONS: 冒頭のスクショ注記を「差し込み済み・未差し込み◯か所はマーカーのまま」に更新。
  - パス検証: 全ドキュメントの相対リンク・画像パスを実ファイルと照合 → `MISS` ゼロ。
- **リポジトリを public 化する方針になったため、LINE userId 入りの画像を削除**。
  - フェーズ20 で「本人テスト垢・private リポジトリだから許容」と判断していた
    `IMG_6261` / `IMG_6262`（ログ一覧、`line_user_id` が写っている）を
    `git rm` で削除。OPERATIONS No.14（ログの一覧）は本文・付録Aとも `📷` マーカーに戻し、
    「お客様の記号は必ずぼかす」「加工してから差し込む」と明記。
  - 結果、未差し込みは **4点**（No.2 Android ホーム追加 / No.14 ログ一覧 / No.16 公式
    アカウントのチャット / No.19 配信完了表示）。`docs/images/` は 22 → 20 ファイル。
  - 本文の画像参照17行はすべて実在確認済み。

### 学び（追加分）

- 「private だから許容」で入れた情報は、public 化の判断が出た瞬間に棚卸しが要る。
  最初から public 前提で「本人のものでも userId は載せない」を基準にしておけば、
  差し替え作業が発生しなかった。可視性の変更は後から起きうるものとして扱う。
